...
class ByteStreamer:
    ...
    async def get_file_properties(self, message_id: int, type_channel=None) -> FileId:
        """
        ...
        if message_id not in self.cached_file_ids:
            await self.generate_file_properties(message_id, type_channel)
            logger.debug(f"Cached file properties for message with ID {message_id}")
        return self.cached_file_ids[message_id]
    
    async def generate_file_properties(self, message_id: int, type_channel=None) -> FileId:
        """
        Generates the properties of a media file on a specific message.
        returns ths properties in a FIleId class.
        """
        tchannel = Var.BIN_CHANNEL if type_channel == None else type_channel
        file_id = await get_file_ids(self.client, tchannel, message_id)
